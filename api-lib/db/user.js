...
export async function findUserForAuth(db, userId) {
  ...
  // user.department = await db.collection('departments').findOne({"_id":user.department});
  // console.log(user);
  return user || null;
}
...
export async function insertUser(
  db,
  { email, originalPassword, bio = '', name, profilePicture, username, department }
) {
  const user = {
    emailVerified: false,
    profilePicture,
    ...
    username,
    bio,
    department: new ObjectId(department._id),
    isAdmin: false
  };
