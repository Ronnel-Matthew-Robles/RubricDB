...
import { findDepartmentById } from '@/api-lib/db';
...
handler.post(
  ...
  ...auths,
  async (req, res) => {
    let { username, name, email, password, department } = req.body;
    ...
    if (emailDomain !== "lpulaguna.edu.ph") {
      res
        .status(400)
        .json({ error: { message: 'The email you entered is not an official LPU email address.' } });
      return;
    }

    const dept = await findDepartmentById(req.db, department);
