...
handler.get(async (req, res) => {
  const unpublishedStatus = await findStatusByName(req.db, "Unpublished");
  const rubrics = await findRubrics(req.db, null, null, null, unpublishedStatus._id + '');
...
handler.post(async (req, res) => {
  ...
  const updated = await setStatusOfRubric(req.db, rubric._id, "Unpublished");

  res.json({ isUnpublished: updated });
});
